	...
	if "identity" in log and log["type"] == "disassociation":
		...
		if ap_status["ap_user_count"] > 0 and identity in ap_status["clients_identity"]:
			...
			try:
				...
				#write on DB
				write_on_db(ap_name, new_ap_user_count)
...
on the DB
"""
def write_on_db(ap_name, new_ap_user_count):
	#write on DB
	db = DBClient()

	# Write some data
	db.PeopleCountWrite(ap_name, new_ap_user_count)
	...
	#print()
	#print (result)
	print ("DB updated with {}, {}".format(ap_name, new_ap_user_count))
	return
